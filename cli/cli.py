...
def load_config(config_path=None):
    """Load configuration from a YAML file in the current directory or specified path, with fallback defaults."""
    default_db_config = {
    ...
    # Check current directory for config.yaml if no config_path is provided
    if not config_path:
    ...
    if os.path.exists(config_path):
        ...
        return {
            ...
            "password": db_config.get("password", default_db_config["password"]),
            ...
            "api-key": llm_config.get("api-key", default_llm_config["api-key"]),
    ...
    return {
        ...
        "api-key": default_llm_config["api-key"],
...
@click.option('--host', default=lambda: load_config().get('host'), help='Database host')
@click.option('--port', default=lambda: load_config().get('port'), help='Database port')
@click.option('--dbname', default=lambda: load_config().get('dbname'), help='Database name')
@click.option('--user', default=lambda: load_config().get('user'), help='Database user')
@click.option('--password', prompt=True, hide_input=True, help='Database password')
@click.option('--provider', default=lambda: load_config().get('provider'), help='LLM provider (e.g., openai, gemini, deepseek, groq)')
@click.option('--api-key', prompt=True, hide_input=True, default=lambda: load_config().get('api-key'), help='LLM API key')
@click.option('--model', default=lambda: load_config().get('model'), help='LLM model')
@click.option('--max-rows', default=lambda: load_config().get('max_rows', 1000), help='Maximum rows to return')
...
def cli(ctx, config, host, port, dbname, user, password, provider, api_key, model, max_rows, read_only):
    ...
    if config:
        ...
        api_key = config_data.get('api-key', api_key)
